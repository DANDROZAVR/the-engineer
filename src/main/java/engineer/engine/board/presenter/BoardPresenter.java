...
public class BoardPresenter {
    ...
    private final static int defMaxFieldWidth = 50, defMaxFieldHeight = 50, defMinFieldWidth = 5, defMinFieldHeight = 5, defFieldWidth = 30, defFieldHeight = 30;
    private final double minFieldWidth, minFieldHeight, maxFieldWidth, maxFieldHeight;
    private Rectangle cam;

    final static double speedChangingSize = 1.1;

    public BoardPresenter(Board board) {
        this(board, defMaxFieldWidth, defMaxFieldHeight, defMinFieldWidth, defMinFieldHeight);
    }
    public BoardPresenter(Board board, double maxFieldWidth, double maxFieldHeight, double minFieldWidth, double minFieldHeight) {
        this.board = board;
        ...
        this.fieldWidth = defFieldWidth;
        this.fieldHeight = defFieldHeight;
    ...
    public void startObserve() { board.addObserver(this::onFieldChange); }
    public void stopObserve() { board.removeObserver(this::onFieldChange); }
    ...
    public void moveCamera(double deltaX, double deltaY) {
        cam.setX(checkCamBorders(cam.getX() + deltaX, 0, fieldWidth * getRows() - cam.getWidth())); //check
        cam.setY(checkCamBorders(cam.getY() + deltaY, 0, fieldHeight * getColumns() - cam.getHeight())); //check
        if (drawRequest != null) {
            try {
                status.checkVisibleFields(drawRequest);
            } catch (IndexOutOfBoardException e) {
    ...
    private class BoardDrawableStatus {
        ...
        public void checkVisibleFields(DrawRequest drawRequest) throws IndexOutOfBoardException {
            ...
            for (int r = Math.max(0, visibleRowStart - 2); r <= Math.min(visibleRowStart + 2 + camRows, getRows() - 1); ++r)
                for (int c = Math.max(0, visibleColumnStart - 2); c <= Math.min(visibleColumnStart + 2 + camCols, getColumns() - 1); ++c) {
                    if (!wasDrawn[r][c]) {
