...
public class GameGui {
  ...
  public GameGui(Stage window, TextureManager textureManager, ExitGameCallback exitGameCallback) {
    ...
    Canvas tempContextMenu =
        new Canvas(miniMapWidth, mapHeight - miniMapHeight); // will be removed later. just for now
    ...
    // if we want to make windows resizable, think about binding the width and the height (just
    // callbacks)
    ...
    root.getChildren()
        .addAll(mainHBox, pauseTextImgView, pauseImgView, stopImgView, button, button2);
  ...
  public void start(Runnable runnable) {
    ...
    window
        ...
        .setOnKeyPressed(
            e -> {
              if (!pausing) boardGui.getKeyHandler().handleKey(e.getCode(), true);
              e.consume();
            });
    window
        .getScene()
        .setOnKeyReleased(
            e -> {
              if (!pausing) boardGui.getKeyHandler().handleKey(e.getCode(), false);
              e.consume();
            });
  ...
  private void addMouseHandler(Canvas root, EventType<MouseEvent> eventType) {
    ...
    root.addEventHandler(
        eventType,
        e -> {
          if (!pausing) boardGui.onMouseEvent(eventType, e);
          e.consume();
        });
  }
}
